    ...
    public class GuardAI3D : MonoBehaviour
    {
        ...
        private void MoveTowards(Vector3 target, float speed, float minDistance = 0)
        {
            transform.position = Vector3.MoveTowards(
                transform.position,
                target,
                Mathf.Max(0, Mathf.Min(speed * Time.deltaTime, Vector3.Distance(transform.position, target) - minDistance))
            );
        }
        ...
        private IEnumerator Search()
        {
            ...
            while (true)
            {
                ...
                yield return MoveToPosition(
                    (Vector3)transform.position //+ Random.insideUnitCircle * 10,
                    ,patrolSpeed
                );
            }
        }
